...
def rem_dream_walk(cluster_entities: list[str] = None) -> list[dict]:
    """
    REM 梦游: 从 Neo4j 高重要性节点做随机游走，发现隐含关联
    
    P10 增强:
    - 种子来源混合: Neo4j high-degree + 当前cluster实体 (优先)
    - cluster实体有新鲜事实基础，梦游更有针对性
    
    Returns: [{"source": ..., "target": ..., "path": [...], "type": "DREAM_WALK"}]
    """
    try:
        from neo4j import GraphDatabase
    except ImportError:
    ...
    try:
        driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASS))
        with driver.session() as session:
            # P10: 种子来源混合 — cluster实体优先 + Neo4j high-degree补充
            cluster_seed_names = []
            if cluster_entities:
                for ent in cluster_entities[:3]:
                    r = session.run("MATCH (n:Concept {name: $name}) RETURN n.name as name, COUNT { (n)--() } as degree", name=ent).data()
            ...
            if remaining_slots > 0:
                ...
                neo4j_seeds = session.run("""
                    ...
                    AND NOT n.name IN ['user', 'assistant', 'system', 'from', 'with',
                ...
                """, seed_count=remaining_slots, exclude=exclude_names).data()
            ...
            log.info(f"  💭 REM 梦游: {len(seeds)} 个种子节点")
            ...
            for seed in seeds:
                ...
                # 路径中非相邻的节点对 = 梦游发现的隐含关联
                if len(path) >= 3:
                    for i in range(len(path)):
                        for j in range(i + 2, len(path)):  # 跳过直接邻居
                            new_edges.append({
                                "source": path[i],
                                "target": path[j],
                                "path": "->".join(path[i:j+1]),
                                "type": "DREAM_WALK",
                                "confidence": 0.3,  # 梦游关联置信度低
                            })
        
        driver.close()
    except Exception as e:
        log.warning(f"⚠️ REM 梦游失败: {e}")
    ...
    if unique_edges:
        log.info(f"  💭 REM 梦游发现 {len(unique_edges)} 条隐含关联")
        for e in unique_edges[:3]:
            log.info(f"    {e['source']} → {e['target']} (via {e['path']})")
