...
from dream_cycle.entities import _is_valid_entity, _KEYWORD_DOMAIN_BOOST
...
def stage3_deep_sleep(
    ...
    for item in rem_results.get("dedup_candidates", []):
        ...
        log.info(
            f"  🗑️ 去重→归档: remove={remove_id[:8]} (sim={sim:.3f}, keep={keep_id[:8]})"
        )
    ...
    if rem_results.get("cross_cluster_relations"):
        log.info(
            f"  🔗 跨聚类共现: {len(rem_results['cross_cluster_relations'])} 条新关系"
        )
            ...
            for j in range(i + 1, min(i + 3, len(valid_keywords))):
                ...
                conn.execute(
                    ...
                    (
                        ...
                        rel_type,
                        0.4,
                        "dream_keyword_cooccurrence",
                    ),
                )
                stats["inferred"] += 1
                neo4j_relations.append(
                    {
                        "source": valid_keywords[i],
                        "target": valid_keywords[j],
                        "type": rel_type,
                        "confidence": 0.4,
                    }
                )
    ...
    if neo4j_relations and not dry_run:
        ...
        log.info(
            f"  🔗 Neo4j 回写: {neo4j_written}/{len(deduped_relations)} 关系 "
    ...
    if not dry_run:
        ...
        if shy_stats["total"] > 0:
            log.info(
                f"  🧬 SHY: {shy_stats['protected']} protected, "
    ...
    if contradictions:
        ...
        for c in contradictions[:5]:
            log.info(
                f"    {c['marker']}: {c['mem1']['id'][:8]} vs {c['mem2']['id'][:8]}"
            )
    ...
    health = {
        "freshness": min(
            1.0, (total_memories - archived_count) / max(total_memories, 1)
        ),
        ...
        "reachability": min(
            1.0, stats.get("inferred", 0) / max(total_memories * 0.5, 1)
        ),
    }
    health_score = (
        sum(
    ...
    for item in rem_results.get("vault_candidates", []):
        ...
        conn.execute(
            ...
            (
                ...
                entity_name,
                category,
                len(item.get("memories", [])),
                f"score={item.get('best_score', 0):.2f}",
            ),
        )
    ...
    log.info(
        f"  ✅ Deep Sleep 完成: deduped={stats['deduped']}, merged={stats['merged']}, "
...
def detect_slot_conflicts() -> list[dict]:
    """
    语义签名冲突检测: 批量查询向量邻居 + 文本, 避免 N+1 子进程调用

    Returns: [{"mem1": ..., "mem2": ..., "slot_similarity": ..., "value_diff": ...}]
    """
    conflicts = []

    # 1. 取最近 7 天有文本的记忆 ID + 文本 (最多 200 条, 1 次 docker exec)
    recent_rows = pg_query("""
        SELECT id::text, LEFT(payload->>'data', 300)
        FROM mem0
    ...
    recent_id_list = list(text_map.keys())
    if len(recent_id_list) < 2:
        log.info("  ✅ 近期记忆不足 2 条, 跳过冲突检测")
        return []

    # 2. 批量查向量邻居 (1 次 docker exec, 自连接 + IN 过滤)
    id_values = ",".join(f"'{mid}'" for mid in recent_id_list)
    neighbor_rows = pg_query(f"""
        SELECT a.id::text AS src, b.id::text AS tgt,
               ROUND((a.vector <=> b.vector)::numeric, 4) AS dist
          FROM mem0 a, mem0 b
         WHERE a.id::text IN ({id_values})
           AND b.id::text IN ({id_values})
           AND a.id < b.id
    ...
    if not neighbor_rows:
        log.info("  ✅ 无向量近邻, 跳过冲突检测")
        return []

    # 3. 在 Python 里做配对 + 文本相似度 (纯内存, 无 docker exec)
    for r in neighbor_rows:
        if len(r) < 3:
            continue
        mem_id, nid, dist_str = r[0], r[1], r[2]
        dist = safe_float(dist_str)
        if dist is None:
            continue

        text1 = text_map.get(mem_id, "")
        text2 = text_map.get(nid, "")
    ...
    if conflicts:
        ...
        for c in conflicts[:3]:
            log.info(
                f"    [{c['slot_similarity']:.2f}槽似, {c['value_diff']:.2f}值差] "
...
def resolve_slot_conflicts(
    ...
    for c in high_diff[:max_resolve]:
        ...
        v = llm_verify_contradiction(
            mem1_text,
            mem2_text,
            f"slot_conflict(sim={c.get('slot_similarity', 0):.2f},diff={c.get('value_diff', 0):.2f})",
        )
        ...
        if ctype == "SUPERSEDE":
            ...
            # 获取两条记忆的创建时间
            rows1 = pg_query(
                f"SELECT id::text, payload->>'created_at' FROM mem0 WHERE id::text = '{mem1_id}'"
            )
            rows2 = pg_query(
                f"SELECT id::text, payload->>'created_at' FROM mem0 WHERE id::text = '{mem2_id}'"
            )
    ...
    conn_cl = sqlite3.connect(str(DREAM_DB), timeout=30.0)
    for r in resolved:
        ctype = r["type"]
    ...
    log.info(
        f"  ✅ P7 冲突处理: {len(resolved)} 条 (SUPERSEDE={sum(1 for r in resolved if r['type'] == 'SUPERSEDE')}, "
