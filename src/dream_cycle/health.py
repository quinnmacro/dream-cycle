...
def check_dream_trigger() -> dict:
    ...
    if len(new_memories) >= TRIGGER_MIN_NEW_MEMORIES:
        ...
        urgency = (
            "high" if len(new_memories) >= TRIGGER_MIN_NEW_MEMORIES * 3 else urgency
        )

    # 3. 冲突密度检查 (从 contradiction_log)
    try:
        conn = sqlite3.connect(str(DREAM_DB))
        total_contra = conn.execute(
            "SELECT COUNT(*) FROM contradiction_log"
        ).fetchone()[0]
        pending_contra = conn.execute(
        ...
        if total_contra > 0:
            ...
            if conflict_density >= TRIGGER_CONFLICT_DENSITY:
                reasons.append(
                    f"conflict_density_{conflict_density:.2f}>={TRIGGER_CONFLICT_DENSITY}"
                )
                urgency = "high"
    except Exception:
        pass  # 表可能不存在 (首次运行)
        ...
        if len(recent) > 50:
            ...
            if mean_len > 0:
                ...
                if concentration >= TRIGGER_MEMORY_ENTROPY:
                    reasons.append(
                        f"entropy_{concentration:.2f}>={TRIGGER_MEMORY_ENTROPY}"
                    )
...
def show_health_dashboard():
    ...
    # Manifest 统计
    total_manifest = conn.execute("SELECT COUNT(*) FROM processed_manifest").fetchone()[
        0
    ]
    active_manifest = conn.execute(
        "SELECT COUNT(*) FROM processed_manifest WHERE status='active'"
    ).fetchone()[0]
    archived_manifest = conn.execute(
        "SELECT COUNT(*) FROM processed_manifest WHERE status='archived'"
    ).fetchone()[0]

    # Relation 统计
    total_relations = conn.execute("SELECT COUNT(*) FROM relation_log").fetchone()[0]
    useful_relations = conn.execute(
        "SELECT COUNT(*) FROM relation_log WHERE confidence >= 0.5"
    ).fetchone()[0]
    boosted_relations = conn.execute(
        "SELECT COUNT(*) FROM relation_log WHERE confidence >= 0.6"
    ).fetchone()[0]
    cross_cluster_rels = conn.execute(
        "SELECT COUNT(*) FROM relation_log WHERE method = 'cross_cluster_cooccurrence'"
    ).fetchone()[0]
    ...
    try:
        ...
        with driver.session() as session:
            ...
            neo4j_dream = session.run(
                "MATCH ()-[r]->() WHERE r.source = 'dream_cycle' RETURN count(r)"
            ).single()[0]
        driver.close()
    except Exception:
        pass
    ...
    print(
        f"  Manifest: {active_manifest} active / {archived_manifest} archived / {total_manifest} total"
    )

    print("\n🔗 **关系网络**")
    print(
        f"  总关系: {total_relations} | 高置信(>=0.5): {useful_relations} | LLM Boost(>=0.6): {boosted_relations}"
    )
    print(f"  跨聚类: {cross_cluster_rels} | Neo4j: {neo4j_dream}/{neo4j_total}")

    print("\n📝 **Vault 沉淀**")
    print(
        f"  建议: {total_suggestions} | pending: {pending_suggestions} | auto_created: {auto_created} | reviewed: {reviewed}"
    )

    print("\n⚡ **冲突检测**")
    print(
        f"  总冲突: {total_contra} | 已解决: {resolved_contra} | pending: {total_contra - resolved_contra}"
    )
    ...
    coverage = (
        min(1.0, neo4j_dream / max(mem_count * 0.1, 1))
        if neo4j_dream > 0
        else min(1.0, (useful_relations + auto_created) / max(total_manifest * 0.5, 1))
    )

    # coherence: 矛盾解决率
    coherence = (
        min(1.0, 1.0 - (total_contra - resolved_contra) / max(total_contra, 1))
        if total_contra > 0
        else 1.0
    )
    ...
    # efficiency: 处理效率
    efficiency = min(
        1.0, 1.0 - max(0, total_manifest - active_manifest) / max(total_manifest, 1)
    )

    # reachability: Neo4j总关系 vs 记忆量 (目标: 30% 连接有向边)
    reachability = (
        min(1.0, neo4j_total / max(mem_count * 0.3, 1))
        if neo4j_total > 0
        else min(1.0, useful_relations / max(total_manifest * 0.3, 1))
    )
    ...
    print(f"\n💊 **综合健康: {score:.0f}/100**")
    print(
        f"  cov={health['coverage']:.0%} coh={health['coherence']:.0%} eff={health['efficiency']:.0%} reach={health['reachability']:.0%}"
    )
    print("=" * 50)
